...
void store.dispatch('questionnairePatient/setQuestionnaireExamenFisico',props.questionnaireId);
const questionnaire = computed(()=>store.getters['questionnairePatient/getQuestionnaireExamenFisico']);
  ...
  currentValue?.resource?.item.forEach(item=>{
    if(item.item){
        item.item.forEach(subItem=>{
          ...
          objectComponent.push(component(subItem.text,answer(subItem.answer[0]),extension,subItem.linkId))
        })
    }else{
      let itemExten = []
      let extension = null
      ...
      objectComponent.push(component(item.text,answer(subItem.answer[0]),extension,item.linkId))
    }
  })
  const observation = {
...
watch(formData, (currentValue, oldValue) => {
  currentValue.data['39156-5'] = currentValue.data['29463-7'] && currentValue.data['8302-2']? currentValue.data['29463-7']/Math.pow((currentValue.data['8302-2']/100),2):''
  ...
  <q-card class="my-card q-mb-sm" flat bordered>
    ...
    <q-card-section horizontal>
      <q-card-section class="col-12">
        <Questionnaire :status="status"  :observationResource="observationResource"  :questionnaire="questionnaire" :modelValue="formData" @update:modelValue="newValue => formData = newValue" :questionnaireId="questionnaireId" :patientId="patientId" :questionnaireResponse="questionnaireResponse"/>
      </q-card-section>
    </q-card-section>
  </q-card>
</template>
