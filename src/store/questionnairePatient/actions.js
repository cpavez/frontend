import api from "../../config/axiosHAPI";
const actions = {
    ...
    async getQuestionnaireResponsePatient({ commit },payload){
        console.log(commit)
        try {
            const response = await api.get('QuestionnaireResponse?subject:Patient.identifier=http://registrocivil.cl/Validacion/RUN|'+payload)
            return response
        }catch (e){
            return e.response
        }
    ...
    postQuestionnaire({ commit },payload){
        console.log(commit)
        void api.post('QuestionnaireResponse',payload)
            .then((response)=>{
                return response
            })
